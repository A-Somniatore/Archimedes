...
///
/// ```rust,ignore
/// use archimedes_macros::HandlerBinder;
///
...
mod tests {
    ...
    fn test_binder_register_unknown_operation() {
        ...
        let handler = Box::new(|_ctx| Box::pin(async { Ok(Default::default()) }));
    ...
    fn test_binder_duplicate_handler() {
        let mut binder = HandlerBinder::new(vec!["op1"]);
        let handler1 = Box::new(|_ctx| Box::pin(async { Ok(Default::default()) }));
        let handler2 = Box::new(|_ctx| Box::pin(async { Ok(Default::default()) }));
